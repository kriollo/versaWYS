...
import { DATE_FORMAT, HTTP_STATUS, MYSQL_BOOLEAN, PAGINATION, TIME_CONVERSIONS, TIMEOUTS, } from '@/dashboard/js/constants';
/**
 ...
 */
export const convertDataTypes = (data, conversions) => {
    if (!data || typeof data !== 'object') {
        return data;
    }
    // Si es un array, aplicar conversión a cada elemento
    if (Array.isArray(data)) {
        return data.map(item => convertDataTypes(item, conversions));
    }
...
export const getDateToday = () => {
    ...
    const mes = String(fecha.getMonth() + DATE_FORMAT.MONTH_OFFSET).padStart(DATE_FORMAT.PAD_LENGTH, DATE_FORMAT.PAD_CHAR);
...
export const addDias = (fecha, dias) => {
    ...
    const { year, month, day } = {
        ...
        month: String(fechaActual.getMonth() + DATE_FORMAT.MONTH_OFFSET).padStart(DATE_FORMAT.PAD_LENGTH, DATE_FORMAT.PAD_CHAR),
...
export const versaAlert = async (Params) => {
    const { title = '¡Éxito!', message = '', html = '', type = 'success', AutoClose = true, callback, customClass = {}, } = Params;
