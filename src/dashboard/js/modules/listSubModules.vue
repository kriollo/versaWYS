...
const changePosition = async (/** @type {Object} */ item) => {
    const { value: position } = await Swal.fire({
        ...
        input: 'number',
        inputValue: Number(item.posicion) + (item.direction === 'up' ? -1 : 1),
        inputAttributes: {
            min: 1,
        },
        showCancelButton: true,
...
<template>
    <modal :showModal="showModal" idModal="modalSubModule" size="max-w-7xl" @accion="accion">
        <template #modalTitle>
            <div class="flex justify-between">
                <h3 class="text-lg font-medium text-gray-900 dark:text-white">
                    <i class="bi bi-menu-app-fill text-2xl text-blue-500  dark:text-blue-300"></i>
        ...
        <template #modalBody>
            <customTable @accion="accion" v-if="showModal" id="subModulesTable" tablaTitle="Listado de Sub Modulos"
                urlData="/admin/modules/getSubModules" :externalFilters="'id_menu=' + idModule" fieldOrder="posicion"
