...
<template>
    <div class="w-full mx-auto rounded-lg shadow-md overflow-hidden">
        <div class="px-4 py-2">
            <inputEditable v-model="perfil.nombre" id="nombre" field="nombre" type="text"
                placeholder="Nombre del perfil" @accion="accion" />
            <hr class="my-4" />
        </div>
        <div class="px-6 text-gray-700 dark:text-white">
            <div class="flex justify-between">
                <h2 class="text-lg font-semibold">Pagina de Inicio:</h2>
                <select class="w-[80%] p-2 border border-gray-300 rounded-lg bg-white dark:bg-gray-800"
                    ...
                    <ul class="ml-4">
                        <li v-for="(menu, index) in seccion" :key="index" class="py-2 flex gap-2">
                            <svg class="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white"
                                aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" :fill="menu.fill_menu === '1'
                                    ? 'currentColor'
                                    : 'none'
                                    " viewbox="0 0 24 24" v-html="removeScapeLocal(menu.icon)"></svg>
                                ...
                                <ul class="ml-2">
                                    <li v-for="(submenu, i) in menu.submenu" :key="i" class="flex gap-2">
                                        ...
                                        <div class="flex justify-between gap-2 w-full">
                                            <label class="block text-sm font-medium text-gray-900 dark:text-white" :for="submenu.id_menu +
                                                '_' +
                                                submenu.id_submenu
                                                ">
                                                {{ submenu.submenu }}
                                            </label>
                                            <input :id="submenu.id_menu +
                                                '_' +
                                                submenu.id_submenu
                                                " type="checkbox"
                            ...
                            <div v-else class="w-full">
                                <div class="flex justify-between gap-2">
                                    ...
                                    <input :id="menu.id_menu" type="checkbox"
                                        class="form-checkbox h-5 w-5 text-primary-600 dark:text-primary-400 focus:ring-primary-500 dark:focus:ring-primary-500"
