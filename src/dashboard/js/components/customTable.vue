...
<template>
    <div class="flex flex-col">
        ...
        <div
            ...
            <dropDown v-model="data.meta.per_page" key="per_page" title="Mostrar" from="per_page"
                :list="showPerPages" />

            <dropDown @update:model="accion" v-model="modelExcel" key="excel" from="excel"
                :list="['Exportar Página', 'Exportar todo']" />

            <!-- input buscar -->
            <div class="relative">
                <button class="absolute inset-y-0 rtl:inset-r-0 end-0 flex items-center pe-3 cursor-pointer"
                    @click="setFilter" title="Click para buscar">
                ...
                <button v-if="data.meta.filter"
                    class="absolute inset-y-0 rtl:inset-r-0 end-5 flex items-center pe-3 cursor-pointer"
                    @click="clearFiler" title="Limpiar filtro">
                    <svg class="w-4 h-4 text-gray-500 dark:text-gray-400 hover:text-orange-900 dark:hover:text-orange-900"
                        fill="none" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                ...
                <input type="text"
                    ...
                    :id="idTable + '_table-search-users'" @keyup.enter="setFilter"
                    placeholder="Ingrese y presione 'Enter' para buscar" v-model="data.meta.filter" />
            </div>
        </div>
        <div class="px-2 overflow-x-auto">
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        <th :colspan="col.col" scope="colspan" v-for="col in data.colspan">
                ...
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th class="px-4 py-2" scope="col" v-for="col in data.columns">
                            {{ col.title }}
                            <button
                                ...
                                <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path :d="data.meta.order[0] === col.field &&
                                        data.meta.order[1] === 'asc'
                                        ? 'M12 14l-4-4-4 4M12 10v8'
                                        : 'M12 10l4 4 4-4M12 14v-8'
                                        " :fill="data.meta.order[0] === col.field
                                            ? 'currentColor'
                                            : 'currentColor'
                                            " :stroke="data.meta.order[0] === col.field
                                                ? 'currentColor'
                                                : 'currentColor'
                                                " stroke-width="1" />
                                </svg>
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700" :key="row.id"
                        v-for="(row, index) in data.data">
                        <td class="px-4 py-4" :key="col.field" v-for="col in data.columns">
                            <!--status-->
                            ...
                            <div class="flex justify-center" v-else-if="col.type == 'svg'">
                                <svg class="w-[20px] h-[20px] text-gray-800 dark:text-white" :fill="row['fill'] === '1'
                                    ? 'currentColor'
                                    : 'none'
                                    " height="24" v-html="removeScapeLocal(row[col.field])" viewBox="0 0 24 24"
                                    width="24" xmlns="http://www.w3.org/2000/svg"></svg>
                            </div>
                            <!--actions-->
                            <div class="flex justify-center gap-2" v-else-if="col.type == 'actions'">
                                <div v-for="action in col.buttons">
                                    <button :class="computedActionClass(action, row)
                                        " :key="action.id" :title="action.title" @click="
                                            accion({
                                                item: row,
                            ...
                            <!--position-->
                            <div class="flex justify-between" v-else-if="col.type == 'position'">
                                <span>{{ row[col.field] }}</span>
                                <div class="flex items-center justify-center gap-1">
                                    <div v-for="action in col.buttons">
                                        <button :class="action.class" :key="action.id" :title="action.title" @click="
                                            accion({
                                                item: {
                                                    ...row,
        ...
        <nav class="grid justify-center md:content-center md:flex md:justify-between my-4 w-full px-2">
            <span class="text-sm font-normal text-gray-500 dark:text-gray-400 h-8 flex items-center gap-1">
                Monstrando
            ...
            <ul class="inline-flex -space-x-px rtl:space-x-reverse text-sm h-8">
                ...
                <li v-if="getLimitPages[0] > 1">
                    <a class="flex items-center justify-center px-3 h-8 text-blue-600 border border-gray-300 bg-blue-50 hover:bg-blue-100 hover:text-blue-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white cursor-pointer"
                        @click="changePage(1)">
                        1
                    </a>
                </li>
                ...
                <li v-for="page in getLimitPages">
                    <a class="flex items-center justify-center px-3 h-8 text-blue-600 border border-gray-300 bg-blue-50 hover:bg-blue-100 hover:text-blue-700 dark:border-gray-700 dark:bg-gray-700 dark:text-white cursor-pointer"
                        @click="changePage(page)" aria-current="page" v-if="data.meta.page == page">
                        {{ page }}
                    </a>
                    <a class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white cursor-pointer"
                        @click="changePage(page)" v-else>
                        {{ page }}
                    </a>
                </li>
                ...
                <li>
                    <a class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white"
                        :class="data.meta.total_pages <= data.meta.page ||
