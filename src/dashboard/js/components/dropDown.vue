...
<template>
    <div>
        ...
        <button :id="'dropdownButton' + from" :data-dropdown-toggle-component="'dropdownList' + from"
        ...
        <!-- Dropdown menu -->
        <div :id="'dropdownList' + from"
            class="z-10 bg-white rounded-lg shadow dark:bg-gray-700 hidden transition-opacity duration-300 ease-in-out opacity-0">
            <ul class="text-sm text-gray-700 dark:text-gray-200 grid justify-center cursor-pointer"
                aria-labelledby="dropdownPerPage">
                <li v-for="(item, index) in list">
                    <a class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white hover:rounded-lg"
